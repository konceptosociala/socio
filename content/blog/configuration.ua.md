...
Socio — це достатньо налаштовувана тема, тому ви можете відредагувати багато параметрів веб-сайту у файлі `config.toml`. Усі параметри теми мають префікс `socio_` і знаходяться в розділі `[extra]`

## Основне (General)

* `socio_title` - назва веб-сайту, яка відображається в рядку заголовка поряд із заголовком самої сторінки, наприклад: `My page | My site title`
* `socio_favicon` - шлях до іконки сайту. Може бути відносним або абсолютном (на кшталт https://...)
* `socio_logo` - шлях до логотипу веб-сайту. Він розташований на навігаційній панелі. Якщо не задано, натомість відображається назва веб-сайту

## Навігаційні посилання (Navbar links)

* `socio_navbar_links` - array of links, which are displayed on the navbar. They must have following structure:
    - `{ key = "home", link = "/" }` for regular links. `key` must be a valid [translations key](#pereklad). `link` can be either relative or absolute URL;
...
## Багатомовність (Multilingualism)

* `socio_languages` - array of languages, available in a navbar's **language switcher**. If not set, the switcher is not displayed. To configure website languages instead look at [Translations](#pereklad) section. Example:
    ```toml
    socio_languages = [
...
## Банер на головній сторінці (Homepage banner)
